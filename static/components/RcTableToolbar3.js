import {ref, reactive, mergeProps,toValue,watchEffect} from 'vue'
...
export const RcTableToolbar3 = {
	...
	const snackText = ref('');
	const snackColor = ref('deep-purple-accent-4');
	const snackTimeout = ref(2000);

	const localFileName = ref('export');

	
	watchEffect(() => {
	    if (props.exportFileName !== undefined) {
		localFileName.value = props.exportFileName;
	    }
	})	
	...
	return {
	    snackModel,
	    snackText,snackColor,snackTimeout,
	    childVisibleHeaders,
	...
	hidedefaultload: {type:Boolean, default: true},
	...
	localPage: {
	    get: function() {
		//console.log('rc-table-toolbar3 reading page:',this.page);
		return this.page;
	    },
	    set: function(value) {
		//console.log('rc-table-toolbar3 emitting update:page:',value);
	...
	localItemsPerPage: {
	    get: function() {
		//console.log('rc-table-toolbar3 reading itemsPerPage:',this.itemsPerPage);
		return this.itemsPerPage;
	    },
	    set: function(value) {
		//console.log('rc-table-toolbar3 emitting update:itemsPerPage:',value);
	...
	fullExportFileName() {
	    ////if ((!this.exportFileName)||(this.exportFileName.length==0)) this.childExportFileName = 'export-data';
	    //if ((!this.exportFileName)||(this.exportFileName.length==0)) return 'export-data';
	    //return this.exportFileName+'.'+this.childExportRadioGroup;
	    return this.localFileName+'.'+this.childExportRadioGroup;
	},
	...
	snack(options) {
	    ...
	    this.snackText = options.text;
	    if (options.color) this.snackColor = options.color;
	    if (options.timeout) this.snackTimeout = options.timeout;
	    this.snackModel = true
	},

	snackMsg(str) {
	    if (str) this.snack({text:str.toString()}); //the royal this
	},

	
	exportRaw() {
	    console.log('EXPORT FILE NAME:',this.fullExportFileName);
	    console.log('EXPORT TYPE:',this.childExportRadioGroup);
	    console.log('EXPORT DATA:',this.exportData);
	    if (this.exportData && (this.exportData.length>0)) {
		const worksheet = XLSX.utils.json_to_sheet(this.exportData);
		//if (this.childExportRadioGroup == 'csv') {
		    
		    
		//} else {		    
		    const workbook = XLSX.utils.book_new();
		    XLSX.utils.book_append_sheet(workbook, worksheet, "ACA Report");
		    XLSX.writeFile(workbook, this.fullExportFileName, { compression: true });
		//}
	    } else {
		this.snackMsg('Run Report First (top left button on table toolbar)');
	    }
	},
	...
	//    console.log('rc-table-toolbar3 watch localPage val:',val,' oldVal:',oldVal);
	//},
