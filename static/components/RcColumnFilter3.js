...
export const RcColumnFilter3 = {
	...
	uniqueNames() {
		...
		if (this.stringarrayfield) {
			...
			tarr = this.items.map(item => tmb.nub(this.header.value(item)));
		    } else {
			tarr = this.items.map(item => tmb.nub(_.get(item,this.header.key)));
		    }
	...
	dedupAddFilterAction(val,exclude) {
		...
		if (cfaIndex<0) {
		    console.log('RcColumnFilter3 Adding filterAction:',filterAction);
